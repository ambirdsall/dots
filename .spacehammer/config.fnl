...
(let [localfile (.. hs.configdir "/../.spacehammer/init-local.lua")]
...
;; Main Menu & Config
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(local dispatcher-app (:
                       (or (hs.application.find "Alfred 4") (hs.application.find :Spotlight))
                       :name))
(local dispatcher-app-name ((string.gmatch dispatcher-app "%S+")))

(local menu-items
       [{:key    :space
         :title  dispatcher-app-name
         :action (activator dispatcher-app)}
        {:key   :w
         :title "Window"
