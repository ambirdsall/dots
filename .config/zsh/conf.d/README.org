...
The files herein are sourced via a zsh ~for~ loop over a glob; ideally the load order
/shouldn't/ matter, but since abstractions have a way of leaking when you aren't looking,
filenames use a numerical prefix system to make the order somewhat predictable.
