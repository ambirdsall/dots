...
(look, I was raised by and around a bunch of earnest, wordy people; deal with it.)
...
*** when you want to do emacsy shit like function advice to vimmy shit like =:q=...
We can make it a little nicer to look up the specific functions at hand:
#+begin_src elisp
(map!
 :leader
...
** the four most frustrating words in the english language: web development in emacs [0/2]
First of all:
#+begin_src elisp :tangle no :noweb-ref doom-modules-lang
(javascript
  +lsp
...
*** just a =nodejs-repl= girl / livin' in a =tsx= world...
Okay, so repl-driven development is the bee's fucking knees; and while =node= is great for basic commonjs stuff, =tsx= is just nicer for node.


**** TODO write fn to return (ideally local) path to =tsx= binary if in a typscript mode and said binary is available
