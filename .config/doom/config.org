...
* who doesn't like a nice dashboard
...
** theme this bad boy
Sometimes (usually) I want dark mode; sometimes (presenting, in direct sunlight, or just mixing it up) I want light mode. Let's define some variables to hold nice dark and light themes and a function to toggle between them—that way, I don't need to keep the theme names straight, and I can swap in a new dark or light theme for all downstream logic just by changing up the corresponding variable.

The modus themes are a solid starting point:
#+begin_src elisp
...
The modus themes are tremendously configurable; let's toggle some of those switches:
#+begin_src elisp
...
I usually want dark mode, though, so let's set that at startup:
#+begin_src elisp
...
* programming
** I mean, why would you not do these things?
#+begin_src elisp
...
have time to shop around. Until I do, I don't want to have to manually choose a scheme every single time I (re)open a ~*.scm~ buffer:
#+begin_src elisp
