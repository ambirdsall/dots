...
** okay wait but suppose I want/need to upgrade DankMaterialShell
presumably it will improve with time/effort, you know?

Anyway, you're going to need to rawdog a tty: you can't overwrite =/usr/local/bin/dms= while it's running. So:
0) remember that you open a tty from GDM et al with shortcuts like e.g. =ctrl+alt+F3=
1) log out
2) grab you a tty and run this here CLI command:
#+begin_src sh :tangle no :results silent
~/sbin/update-dms
#+end_src

I had this nonsense, but now they have a proper COPR, so I just use =dnf= instead:
#+begin_src sh :tangle (or "no" "~/sbin/update-dms") :shebang "#!/usr/bin/env bash"
main() {
  local dms_dir="$HOME/.config/quickshell/dms"
  if ! [ -d "$dms_dir" ]; then
    echo "Could not find a dms installation in '$dms_dir', aborting" >&2
    exit 1
  else
    ...
    git pull
    dms kill
    sudo sh -c "curl -L https://github.com/AvengeMedia/danklinux/releases/latest/download/dms-amd64.gz | gunzip | tee /usr/local/bin/dms > /dev/null && chmod +x /usr/local/bin/dms"
    sudo dnf up quickshell-git
    sleep 0.5
    dms run &
    disown
  fi
...
*** TODO define keybindings to kill/(re)start ~dms~
...
** dynamically fetch built-in screen size
This way, each different laptop plays with the office monitors nicely.
...
niri msg --json outputs | jq '."eDP-1".logical.width'
#+end_src

** monitors
*** Laptop stand, and therefore monitor, goes in the middle, i.e. origin.

The y=300 bit is accounting for the lower height of the laptop stand vs the monitor stands.

Logical size queried by evaluating the =laptop-screen-width()= block.
#+begin_src kdl :tangle config.kdl
output "eDP-1" {
    position x=0 y=300
}
...
*** Acer monitor to the left, i.e. shifted its own width left of origin.
Logical size: 1920x1080

#+begin_src kdl :tangle config.kdl
output "Acer Technologies SB220Q 0x11102304" {
    position x=-1920 y=0
}

#+end_src

*** Dell monitor to the right, i.e. shifted the /laptop/ monitor's width to the right.
Logical size: 2560x1440

#+begin_src kdl :tangle config.kdl
output "Dell Inc. DELL U2722D 3W06FH3" {
    position x=<<laptop-screen-width()>> y=-80
}
#+end_src

I can't decide if it's more exasperating or funny that this happens sometimes, but here we are:
#+begin_src kdl :tangle no
...
focus-ring {
    ...
    //
    active-gradient from="#007a33dd" to="#0c2340dd" angle=160 in="oklch shorter hue"
...
* named workspaces
There is a base set of workspaces I always want to have around and named; there's a
different set of ad hoc or computer-specific workspaces that should also be around and
named but should /not/ be git-tracked. So if there's a file
=$HOME/.config/niri/local-workspaces=, its contents will be appended to the list of named
workspaces.
...
Mod+Shift+W hotkey-overlay-title="Display the focused workspace's name" { spawn "~/bin/niri-helper" "display"; }
...
(defn- choose-other-workspace! [title]
  (let [other-workspaces (->> (json/decode ($<_ niri msg --json workspaces))
    ...
    ($<_ echo ,choices | fuzzel --dmenu --use-bold --gamma-correct --prompt-color ,"008853ff" --prompt ,prompt)))

(defn- display-workspace-name []
  (let [workspaces (json/decode ($<_ niri msg --json workspaces))
        active-space ((find |(get $ "is_focused") workspaces) "name")]
    ($ hydratic timed-banner ,active-space)))

(defn- focus-workspace []
  (let [workspaces (json/decode ($<_ niri msg --json workspaces))
        active-space ((find |(get $ "is_focused") workspaces) "name")
        selection (choose-other-workspace! (string "Go from " active-space " to"))]
...
(defn main [& invocation]
  (match invocation
    [_ "display"] (display-workspace-name)
...
** TODO display workspace names in the UI some{how,where}
...
window-rule {
    ...
    // While it is, regrettably, true that some rotten assholes use the number 88
    // as a shiboleth-cum-homage to one of the most obviously evil dipshits in history, I
    // like multiples of eleven and pianos too much to let them have it all to themselves.
    opacity 0.88

    // the `draw-border-with-background` setting below is commented out since it does
    // nothing while `prefer-no-csd` is set; this comment is still here since I'm not yet
    // sure that `prefer-no-csd` is going to work with no shitty deal-breaker edge cases
...
Mod+Tab hotkey-overlay-title="Toggled tabbed display for column" {
...
Mod+Alt+Shift+P hotkey-overlay-title="Turn off displays" { power-off-monitors; }
#+end_src
* untangled example/reference configuration
** output config
#+begin_src kdl :tangle no
output "eDP-1" {
...
Once the next release of niri hits, there will be a mechanism for including other config files, at which point I can run =M-x raise-sexp= on the ~:tangle~ arg's ~(and (file-exists-p ...) ...)~ form, move the contents of =./local-binds.kdl= inside a ~binds { ... }~ in =laptop.kdl=, delete the =sh= block, and be on my merry way. For now, though, shout out to org-mode for making a workaround possible, I guess.

#+begin_src dkl :tangle (or "no" (and (file-exists-p "laptop.kdl") "config.kdl"))
